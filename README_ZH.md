...
<p align="center">
  <strong>让人只关注任务面板，Agent 自动回写进展、流转状态、沉淀会话上下文。</strong>
</p>
...
AI 编程真正消耗人的地方，往往不是“任务在哪里运行”，而是人必须在脑子里维护一张隐形任务表：哪个会话在做什么、哪条评论需要跟进、哪个结果已经验证、哪个任务该进入评审。会话越多，人就越容易在不同窗口、不同历史记录、不同 Agent 输出之间来回翻找。

这种工作方式的问题是：任务状态依赖人的记忆，而不是依赖系统。
...
Codex Taskboard 要改变的是这个范式：不要让人脑记住任务、再去不同会话里找事情；而是让人只关注任务面板上的事项，让 Agent 围绕 Issue 执行、评论、补证据、关联会话，并在完成后把状态自动流转到下一步。

每个 Issue 都是一个可持续更新的工作单元：它保存状态、优先级、标签、负责人、分支、worktree、本地项目路径、评论、附件、知识提案，以及当前和历史运行时会话。你可以从 Issue 或某条评论直接创建新的 Codex / Claude Code / Oh My Pi 会话，也可以把当前会话关联回 Issue。Agent 完成阶段性工作后，把结果、证据、阻塞点和下一步状态写回面板；人只需要看面板上的状态、评论和待处理事项，而不是靠记忆追踪每段会话。

更重要的是，Issue、评论和关联会话会把一次任务从“散落的聊天记录”变成可追溯、可沉淀、可复用的工作记录：为什么这么做、谁处理过、哪条评论触发了返工、哪个会话给出了证据、最后状态如何变化，都能回到同一个任务里查到。
...
- 评论、Issue 和关联会话共同构成任务上下文，把需求、执行、返工、验证和结论串成可追溯记录。
- 支持附件上传、下载、删除。
