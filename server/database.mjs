...
function projectInteractionFromRow(row) {
  return {
    id: row.id,
    projectId: row.project_id,
    workspacePath: row.workspace_path,
...
export class TaskboardDatabase {
  ...
  #migrate() {
    ...
    this.database.exec(`
      ...
      DROP TABLE IF EXISTS project_bot_audit_events;
      DROP TABLE IF EXISTS project_bot_sessions;
      DROP TABLE IF EXISTS project_bot_configs;
      ...
      CREATE TABLE IF NOT EXISTS project_interactions (
        ...
        label TEXT NOT NULL CHECK (label IN ('咨询', '缺陷')),
        sources TEXT NOT NULL DEFAULT '[]',
        created_at TEXT NOT NULL
      );

      CREATE INDEX IF NOT EXISTS project_interactions_project_created
        ON project_interactions(project_id, created_at DESC, id DESC);

      CREATE INDEX IF NOT EXISTS project_interactions_label_created
        ON project_interactions(project_id, label, created_at DESC, id DESC);
    `);
  ...
  resolveProjectByWorkspace(workspacePath) {
    const resolved = path.resolve(workspacePath);
    const projects = this.listProjects()
      .filter((project) => project.workspacePath)
      .map((project) => ({ ...project, resolvedWorkspacePath: path.resolve(project.workspacePath) }))
      .filter((project) => (
        resolved === project.resolvedWorkspacePath
        || resolved.startsWith(`${project.resolvedWorkspacePath}${path.sep}`)
      ))
      .sort((left, right) => right.resolvedWorkspacePath.length - left.resolvedWorkspacePath.length);
    const match = projects[0] ?? null;
    if (!match) return null;
    const { resolvedWorkspacePath, ...project } = match;
    return {
      project,
      matchedBy: resolved === resolvedWorkspacePath ? "exact" : "ancestor",
      workspacePath: resolved,
      projectWorkspacePath: resolvedWorkspacePath,
    };
  }

  createProjectInteraction(input) {
    ...
    this.database.prepare(`
      INSERT INTO project_interactions (
        id, project_id, workspace_path, source_runtime, channel, conversation_id,
        question, answer, label, sources, created_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      id,
      input.projectId,
    ...
    return projectInteractionFromRow(this.database.prepare("SELECT * FROM project_interactions WHERE id = ?").get(id));
  }

  getProjectInteraction(id) {
    const row = this.database.prepare("SELECT * FROM project_interactions WHERE id = ?").get(id);
    return row ? projectInteractionFromRow(row) : null;
  }

  listProjectInteractions(projectId, limit = 50) {
    if (!this.database.prepare("SELECT 1 FROM projects WHERE id = ?").get(projectId)) {
      throw new ApiError(404, "PROJECT_NOT_FOUND", `Project '${projectId}' does not exist`);
    }
    return this.database.prepare(`
      SELECT * FROM project_interactions
      WHERE project_id = ?
      ORDER BY created_at DESC, id DESC
      LIMIT ?
    `).all(projectId, limit).map(projectInteractionFromRow);
  }

  updateProjectInteractionSources(id, sources) {
    const result = this.database.prepare(`
    ...
    if (result.changes !== 1) {
      throw new ApiError(404, "INTERACTION_NOT_FOUND", `Interaction '${id}' does not exist`);
    }
    return this.getProjectInteraction(id);
  }
