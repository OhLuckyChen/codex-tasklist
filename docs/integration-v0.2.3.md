...
- **上游独有文件**（本地无）：`src-tauri/*`(14，Tauri/macOS)、`cloud/*`(6)、`server/cloud-config.mjs`、`server/cloud-proxy.mjs`、`server/project-summary.mjs`、`server/ai-turn-owner.mjs`、`inject/workbuddy-taskboard.user.js` 等。
- **双方都有且都改的重叠文件**：`server/app.mjs`(本地 2684 / 上游 2602)、`server/database.mjs`(2288 / 2121)、`server/ai-chat.mjs`(584/591)、`server/ai-chat-process.mjs`(465/471)、`server/ai-chat-catalog.mjs`(256/273)、`server/index.mjs`(38/41)。
...
| 5 | AI Chat | `server/ai-chat*.mjs`、`server/ai-turn-owner.mjs`(新)、web 面板/思考步骤/拖拽 | **双方都有**，上游更新更全 | **对比采纳**→Phase 2 C4 | app.mjs 路由注册、database.mjs 会话/轮次表 |
...
- **C4 AI Chat**（`ai-chat.mjs` 584/591、`ai-chat-process.mjs` 465/471、`ai-chat-catalog.mjs` 256/273；上游新增 `ai-turn-owner.mjs`）
...
2. 再做**重叠选优**：C2/C3（数据/路由层先稳）→ C1（启动器/线程归属，依赖前两者）→ C4（AI Chat）→ C5（排序，依赖视图）→ C6（knowledge 架构二选一）。
3. 最后评估**未发布修复**按需补迁。
