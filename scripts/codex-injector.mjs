...
function codexIsRunning(appPath) {
  const expectedExecutable = path.join(appPath, "Contents", "MacOS", "ChatGPT");
  ...
  return processes.stdout
    .split("\n")
    .some((command) => command.trim() === expectedExecutable);
}
...
async function main() {
  ...
  try {
    ...
    if (!cdpReachable) {
      ...
      if (codexIsRunning(options.appPath)) {
        throw new Error(
    ...
    while (true) {
      ...
      try {
        ...
        if (!(await isReachable(cdpVersionUrl))) break;
